...
namespace vNekoChatUI.Character.BingUtils.Services
{
        ...
        public BingGptApiClient()
        {
            ...
            var service = new ServiceCollection();
            {
                ...
                }).ConfigurePrimaryHttpMessageHandler(() =>
                {
                    ...
                    {
                        Proxy = DefWebProxy.Instance.GetWebProxy(),
                        //UseProxy = true,
                    };
    ...
    public sealed partial class BingGptApiClient
    {
        ...
        private async Task<BingConversation> SendToBingGPT(string cookie)
        {
            // Http リクエストを用意
            var request = new HttpRequestMessage(HttpMethod.Get, "https://www.bing.com/turing/conversation/create");
            {
                ...
                request.Headers.Add("referer", "https://www.bing.com/search?q=Bing+AI&qs=n&form=QBRE&sp=-1&lq=0");
            ...
            if (true)
            {
                ...
                LogProxy.Instance.Print($"API retry limit exceeded.");
            }
    ...
    public sealed partial class BingGptApiClient
    {
        ...
        /// 返回BingConversation
        /// </summary>
        internal async Task<BingConversation?> WaitReplyAsync(string cookie)
        {
            _bingChatClient = new BingChatClient(new Models.BingChatOption(Enum.CharStyle.Creative, cookie, 1000),
                                                 _client);

            var response = await SendToBingGPT(cookie);
        ...
        internal async Task<string?> ChatAsync(BingRequest message, CancellationToken cancellationToken)
        {
            return await _bingChatClient.ChatAsync(message, cancellationToken);
        }
    }
}
