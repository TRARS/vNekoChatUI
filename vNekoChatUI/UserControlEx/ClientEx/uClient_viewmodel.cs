...
namespace vNekoChatUI.UserControlEx.ClientEx
{
    ...
    internal partial class uClient_viewmodel : NotificationObject
    {
        ...
        //public RelayCommand AmbidextrousCombatSendCommand { get; set; }//前两个AI左右互搏

        public RelayCommand RightClickRevokeAllCommand { get; set; }
        public RelayCommand RightClickReSendCommand { get; set; }
        public RelayCommand RightClickBypassDetectionCommand { get; set; }
        ...
        public RelayCommand BypassDetectionCommand { get; set; }
        ...
        public MenuItemCommand ContinueWithSelectedTextCommand { get; set; }
        public MenuItemCommand ContinueWithSelectedTextExCommand { get; set; }
        public TextBoxPasteCommand PasteParenthesesCommand { get; set; }
        ...
        //
        private bool _chatRoom;
        public bool ChatRoom
        {
        ...
        public ChatMode DebugMode
        {
            ...
            set
            {
                _debugMode = value;
                NotifyPropertyChanged();
                NotifyPropertyChanged(nameof(BingSuggestionVisibility));
            }
        }
        public Visibility BingSuggestionVisibility => (DebugMode is ChatMode.NewBing ? Visibility.Visible : Visibility.Collapsed);
        ...
        //LogText
        //public dynamic LogContent => LogProxy.Instance.LogContent;
        ...
        {
            get { return FlagProxy.Instance.TryBypassDetection; }
            set
            {
                FlagProxy.Instance.TryBypassDetection = value;
                NotifyPropertyChanged();
            }
        }
    }
    ...
    internal partial class uClient_viewmodel
    {
        private void LoadCommand()
        {
            ...
            //右键按钮.撤回以下所有
            this.RightClickRevokeAllCommand = new(para =>
            {
                if (SelectedContact is not null && SelectedContact.Messages.Count >= 1)
                {
                    SelectedContact.IsHistoryChanged = true;
            ...
            //右键按钮,重新发送
            this.RightClickReSendCommand = new(para =>
            {
                if (SelectedContact is not null && SelectedContact.Messages.Count >= 1)
                {
                    SelectedContact.IsHistoryChanged = true;
            ...
            //右键按钮,重新发送（过越狱检测）
            this.RightClickBypassDetectionCommand = new(para =>
            {
                if (SelectedContact is not null && SelectedContact.Messages.Count >= 1)
                {
                    SelectedContact.IsHistoryChanged = true;
            ...
            //this.AmbidextrousCombatSendCommand = new(async para => 
            //{
            //    //if (waiting_reply) { return; }
            //    if (para is not null) { this.UserMessage = $"{para}"; }

            //    var sourceContact = BotContacts[1];// PlayerContact;  //确定发件人 为Player
            //    var targetContact = BotContacts[0]; //SelectedContact;//确定收件人 为当前聊天对象
            //    sourceContact.currentCommConnact = targetContact;
            //    targetContact.currentCommConnact = sourceContact;
            ...
            //            //双AI互搏
            //            sourceContact.MeSayStart(sourceContact, targetContact,  $"{user_input}");
            //        }
            //        //waiting_reply = false;
            //    }
            //});
            ...
            this.LoadBingChatHistoryCommand = new(para =>
            {
                ...
                if (filePaths is not null && filePaths[0].Length > 0)
                {
                    ...
                    if (Array.IndexOf(imageExtensions, extension.ToLower()) != -1)
                    {
                        ...
                        {
                            BmpProxy.Instance.BitmapSourceToPngFile(result, filePath);
                        }
                    }
                    else
                    {
            ...
            //临时过越狱检测
            this.BypassDetectionCommand = new(_ =>
            {
                BypassDetection = !BypassDetection;
                LogProxy.Instance.Print($"TryBypassDetection = {BypassDetection}");
            });

            //取消任务
            this.StopReceivingMessagesCommand = new(para =>
            {
            ...
            this.ContinueWithSelectedTextExCommand = new()
            {
                ...
                HeaderStringFormat = "continue with (BypassDetection)『{0}』"
            };
            ...
            //粘贴个右括号
            this.PasteParenthesesCommand = new TextBoxPasteCommand();
            //选择Bing建议的回复，复制到输入框
            this.SelectSuggestionCommand = new RelayCommand(para =>
            {
                UserMessage = $"{para}";
            });
        }
            ...
            var chatServer = chatPlayground.CreateChatServer(def_server_address, def_server_port, def_server_name);
            {
                ...
                for (int i = 0; i < Math.Min(preload_count, max_count); i++)
                {
                    ...
                    BotContacts.Add(new ContactModel(chatPlayground, $"{obj.name[i]}", true, () => { return (int)this.DebugMode; })//注入BOT专用客户端
                    {
                        ...
                        BypassDetectionDoneAct = () => { BypassDetection = false; },
                ...
                if (true)
                {
                    ...
                    PlayerContact = new ContactModel(chatPlayground, "Player", false, () => { return (int)this.DebugMode; })//注入Player专用客户端
                    {
                        ...
                        ImageBorder = $"./Resources/Icon/vNekoBorder_44_-1,0,0,0.png",
                        ImageBorderSize = 44,
                ...
                //刷新设定按钮的命令在这里
                this.ProfileRefreshCommand = new(_ =>
                {
                    //重新roll一轮
                    obj = BotInit(true);
